...
<div class="container_liste_question">
    <div class="container_question">
        ...
        $intituleFormater=rawurldecode($nomQuestion);
        echo "<div class='container_question1'>";
    echo "<div class='titrelistequestion'>";
        echo "<div id='apparait' class='cacher'>";
        echo "</div>";
        echo "<h1><a href=\"frontController.php?controller=question&action=read&id_question={$_GET['id_question']}\"> Questions " . $intituleFormater  ."</a></h1>";
    echo "</div>";
echo "</div>";
...
foreach ($votants as $votant){
    ...
    $responsableRep = (new UtilisateurRepository())->select($reponse->getIdUtilisateur());
    ...
    $responsableFormater = rawurldecode($responsableRep->getLogin());
    $allTexte = (new TexteRepository())->selectWhere('id_reponse', $repNonFormater);
    if(!empty($allTexte)){

echo "<div class='questions'>";
        echo "<a id='titrequestion' href=\"frontController.php?controller=reponse&action=read&id_reponse={$repFormater}\"> Réponse {$responsableFormater} :\n" . htmlspecialchars("") . " </a>";
